...
// ---------------------------------------------------------------------------------------------------------------------
// Constructor for ProjectorAM1OrderRuyten
// ---------------------------------------------------------------------------------------------------------------------
ProjectorAM1OrderRuyten::ProjectorAM1OrderRuyten( Params &params, Patch *patch ) : ProjectorAM( params, patch )
{
    dt = params.timestep;
...
// ---------------------------------------------------------------------------------------------------------------------
// Destructor for ProjectorAM1OrderRuyten
// ---------------------------------------------------------------------------------------------------------------------
ProjectorAM1OrderRuyten::~ProjectorAM1OrderRuyten()
{
}
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::currents(   ElectroMagnAM *emAM,
                                    Particles &particles,
                                    unsigned int ipart,
                                    double invgf,
                                    int *iold,
                                    ...
                                    bool diag_flag, int ispec)
{
    ...
    double  Sl0[5], Sl1[5], Sr0[4], Sr1[4], DSl[5], DSr[4];
    complex<double>  Jl_p[5], Jr_p[4];
    ...
    double x_n = iold[1*nparts]+j_domain_begin_ ;
    // Ruyten scheme
    Sr0[1] = (1. - delta)*(4*x_n + 2 - delta)/(4.*x_n + 2.);
    ...
    x_n = jp ;
    // Ruyten scheme
    Sr1[jp_m_jpo+1] = (1. - delta)*(5*x_n + 2 - ypn)/(4.*x_n + 2.);
    ...
    jpo -= 1; // Order 1 in R: 4 points stencil from -1 to +2.
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::basicForComplex( complex<double> *rhoj, Particles &particles, unsigned int ipart, unsigned int type, int imode )
{
...
//! Project for diags and frozen species -
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::basicForComplexOnBuffer( complex<double> *rhoj, Particles &particles, unsigned int ipart, unsigned int type, int imode, int bdim0, int bin_shift )
{
    //Warning : this function is not charge conserving.
    // This function also assumes that particles position is evaluated at the same time as currents which is usually not true (half time-step difference).
    // It will therefore fail to evaluate the current accurately at t=0 if a plasma is already in the box.
...
void ProjectorAM1OrderRuyten::axisBC(ElectroMagnAM *emAM, bool diag_flag )
{
...
void ProjectorAM1OrderRuyten::apply_axisBC(std::complex<double> *rhoj,std::complex<double> *Jl, std::complex<double> *Jr, std::complex<double> *Jt, unsigned int imode, bool diag_flag )
{
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::ionizationCurrents( Field *Jl, Field *Jr, Field *Jt, Particles &particles, int ipart, LocalFields Jion )
{
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::ionizationCurrentsForTasks( double */*b_Jx*/, double */*b_Jy*/, double */*b_Jz*/, Particles &/*particles*/, int /*ipart*/, LocalFields /*Jion*/, int /*bin_shift*/ )
{
    // cField2D *JlAM  = static_cast<cField2D *>( Jl );
    // cField2D *JrAM  = static_cast<cField2D *>( Jr );
    // cField2D *JtAM  = static_cast<cField2D *>( Jt );
...
void ProjectorAM1OrderRuyten::currentsAndDensityWrapper( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, bool diag_flag, bool /*is_spectral*/, int ispec, int /*icell*/, int /*ipart_ref*/ )
{
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::currentsAndDensityWrapperOnAMBuffers( ElectroMagn *EMfields, std::complex<double> *b_Jl, std::complex<double> *b_Jr, std::complex<double> *b_Jt, std::complex<double> *b_rhoAM, int bin_shift, int bdim0, Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, bool diag_flag, int /*ipart_ref*/ )
{
    std::vector<int> *iold = &( smpi->dynamics_iold[ithread] );
    std::vector<double> *delta = &( smpi->dynamics_deltaold[ithread] );
    std::vector<double> *invgf = &( smpi->dynamics_invgf[ithread] );
...
// ---------------------------------------------------------------------------------------------------------------------
void ProjectorAM1OrderRuyten::currentsForTasks( ElectroMagnAM */*emAM*/, std::complex<double> *b_Jl, std::complex<double> *b_Jr, std::complex<double> *b_Jt, std::complex<double> *b_rhoAM, Particles &particles, unsigned int ipart, double invgf, int *iold, double *deltaold, std::complex<double> *array_eitheta_old, int bin_shift, int bdim0, bool diag_flag )
{
...
void ProjectorAM1OrderRuyten::susceptibility( ElectroMagn *EMfields, Particles &particles, double species_mass, SmileiMPI *smpi, int istart, int iend,  int ithread, int /*icell*/, int /*ipart_ref*/ )

{
    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {//Loop on bin particles
        ...
        delta  = xpn - ( double )ip;
        delta2 = delta*delta;
        Sl1[0] = 0.5 * ( delta2-delta+0.25 );
        Sl1[1] = 0.75-delta2;
        Sl1[2] = 0.5 * ( delta2+delta+0.25 );

        ypn = r * dr_inv_ ;
        int jp = floor( ypn );
        delta  = ypn - ( double )jp;
        double x_n = jp ;
        Sr1[0] = (1. - delta)*(5*x_n + 2 - ypn)/(4.*x_n + 2.);
        Sr1[1] = 1. - Sr1[0]; //conservation de la charge

        // ---------------------------
        // Calculate the total charge
        // ---------------------------
        ip -= i_domain_begin_ + 1;
        jp -= j_domain_begin_ ;
...
// Projector for susceptibility used as source term in envelope equation
void ProjectorAM1OrderRuyten::susceptibilityOnBuffer( ElectroMagn */*EMfields*/, double *b_ChiAM, int bin_shift, int /*bdim0*/, Particles &particles, double species_mass, SmileiMPI *smpi, int istart, int iend,  int ithread, int /*icell*/, int /*ipart_ref*/ )
{
    // -------------------------------------
    // Variable declaration & initialization
    // -------------------------------------
