...
struct FriedmanFields {
    ~FriedmanFields() {
        ...
        for (unsigned int imode=0; imode<El_.size(); imode++){
            for( unsigned int i=0; i<El_[imode].size(); i++ ) {
                delete El_[imode][i];
            }
            for( unsigned int i=0; i<Er_[imode].size(); i++ ) {
                delete Er_[imode][i];
            }
            for( unsigned int i=0; i<Et_[imode].size(); i++ ) {
                delete Et_[imode][i];
            }
        }
    }
    ...
    std::vector<std::vector<cField *>> El_;
    std::vector<std::vector<cField *>> Er_;
    std::vector<std::vector<cField *>> Et_;
};
